function [LL, N, dsurr] = LLmodelRating(y,D,opts)
    ...
    for t = 1:N
        ...
        s1 = D.S(t,1);        % state 1 state
        s2 = D.S(t,2)+1;      % stage 2 state
        c1 = D.A(t,1);        % choice stage 1
        c2 = D.A(t,2);        % choice stage 2
        r  = D.R(t,1);        % reward
        ...
        % first stage choice
        if opts.generatesurrogatedata == 1                           % is surrogate data is generated, generate choice 1
            c1 = (rand > exp(b1*Q(1))/sum(exp(b1*Q))) +1;            % make choice using softmax
            if trans(t) ; if c1 == 1; s2 = 2; else; s2 = 3; end      % find stage 2 state based on transition
        ...
        % second stage choice
        if opts.generatesurrogatedata == 1                              % is surrogate data is generated, generate choice 2
        ...
        if opts.generatesurrogatedata == 1                      % store latents
            ...
            if mean(D.rewprob(1,:,t)) > 0.2                     % optimal reward at second step
                winState = 2;
            else
                winState = 3;
            end
